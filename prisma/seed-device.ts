...
async function main() {
	...
	const dasList = await prisma.m_das.findMany({ select: { id: true, name: true } });
	if (dasList.length === 0) {
		console.warn('[seed-device] No m_das rows found. Please run prisma:seed:das first.');
		return;
	}

		const tags = await prisma.m_device_tag.findMany({ select: { id: true, name: true }, orderBy: { id: 'asc' } });
	...
	await prisma.m_device.deleteMany({});
		...
		const owners = ['BMKG Pusat', 'BBWS', 'Pemkot Bandung', 'Pemkab Bogor', 'BPBD Jabar'];
	...
	const now = new Date();
		const records = Array.from({ length: 50 }).map((_, i) => {
			...
			const lon = rand(106.5, 108.9);
				...
				last_sending_data: new Date(now.getTime() - Math.floor(rand(0, 7 * 24)) * 3600 * 1000),
